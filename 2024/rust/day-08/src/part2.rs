        ...
        for first in value.clone() {
            for second in value.clone() {
                ...
                if dif != IVec2::ZERO {
                    ...
                    let mut current_dif = first;
                    while (x_minmax.0..=x_minmax.1 - 1).contains(&current_dif.x)
                        && (y_minmax.0..=y_minmax.1 - 1).contains(&current_dif.y)
                    {
                        antinodes.insert(current_dif);
                        current_dif = current_dif + dif;
                    }
                    current_dif = second;
                    while (x_minmax.0..=x_minmax.1 - 1).contains(&current_dif.x)
                        && (y_minmax.0..=y_minmax.1 - 1).contains(&current_dif.y)
